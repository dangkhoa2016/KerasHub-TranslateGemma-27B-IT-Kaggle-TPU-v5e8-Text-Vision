...
class PackagingTests(unittest.TestCase):
    ...
    def test_source_zip_is_independent_of_checkout_timestamps(self):
        root = Path(__file__).resolve().parents[1]
        module = load_module(root / "scripts/package_source.py", "package_source_deterministic")
        with tempfile.TemporaryDirectory() as tmp:
            fake = Path(tmp) / "project"
            source = fake / "src/a.py"
            source.parent.mkdir(parents=True)
            source.write_text("print('hello')\n", encoding="utf-8")
            first = Path(tmp) / "first.zip"
            second = Path(tmp) / "second.zip"

            module.create_source_zip(first, fake)
            stat = source.stat()
            os.utime(source, (stat.st_atime + 86400, stat.st_mtime + 86400))
            module.create_source_zip(second, fake)
